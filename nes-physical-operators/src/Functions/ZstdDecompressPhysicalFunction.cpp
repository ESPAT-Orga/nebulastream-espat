...
VarVal ZstdDecompressPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    if (type.type == DataType::Type::VARSIZED)
    {
        ...
        decompressedSize = nautilus::invoke(
            +[](const int8_t* compressed, size_t inputSize)
            { return static_cast<size_t>(ZSTD_getFrameContentSize(compressed, inputSize)); },
        ...
        nautilus::invoke(
            decompressVarSized, varSizedValueCompressed.getSize(), varSizedValueCompressed.getContent(), decompressedSize, memDecompressed);

        return VariableSizedData{memDecompressed, decompressedSize};
    }
    else
    {

...
/// Reproduction test for "Issue 3: PipeliningPhase mis-compiles operators with multiple consumers"
/// (see MULTI_ROOT_PLAN_ISSUES.md). The physical plan is hand-built (issue 2 currently prevents fan-out
/// graphs from reaching this phase through the normal compile path). The test asserts the EXPECTED
/// pipeline shape and currently FAILS on main: the consumers of a shared operator are appended
/// sequentially into ONE linear pipeline (the second consumer would read the first consumer's output at
/// runtime), with one emit stacked per consumer — silently producing a wrong plan.

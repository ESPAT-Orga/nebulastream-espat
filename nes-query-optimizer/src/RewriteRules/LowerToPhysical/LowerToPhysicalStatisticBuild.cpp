...
getAggregationPhysicalFunctions(const LogicalOperator& logicalOperator, const WindowedAggregationLogicalOperator& aggregation, const QueryExecutionConfiguration& configuration)
{
    std::vector<std::shared_ptr<AggregationPhysicalFunction>> aggregationPhysicalFunctions;
    const auto& aggregationDescriptors = aggregation.getWindowAggregation();
    for (const auto& descriptor : aggregationDescriptors)
    {
        ...
        auto layout = std::make_shared<ColumnLayout>(configuration.pageSize.getValue(), aggregation.getInputSchemas()[0]);
        ...
        if (const auto statisticOperatorOptional = logicalOperator.tryGet<StatisticBuildLogicalOperator>())
        {
            auto statisticOp = statisticOperatorOptional.value();
            if (name.contains("ReservoirSample"))
            {
                ...
                aggregationArguments.numberOfSeenTuplesFieldName = statisticOp.getNumberOfSeenTuplesFieldName();
...
RewriteRuleResultSubgraph LowerToPhysicalStatisticBuild::apply(LogicalOperator logicalOperator)
{
    ...
    auto aggregationPhysicalFunctions = getAggregationPhysicalFunctions(logicalOperator, aggregation, conf);

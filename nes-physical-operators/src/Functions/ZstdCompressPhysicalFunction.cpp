...
VarVal ZstdCompressPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    nautilus::val<int8_t*> valueMem{nullptr};
    ...
    auto memCompressedVarSized = arena.allocateMemory(compressedSize);
    nautilus::memcpy(memCompressedVarSized, tempBufferCompressed, compressedSize);

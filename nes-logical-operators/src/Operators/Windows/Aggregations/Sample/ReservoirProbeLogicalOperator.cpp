...
ReservoirProbeLogicalOperator::ReservoirProbeLogicalOperator(uint64_t statHash, Schema sampleSchema)
    : statHash(statHash), sampleSchema(sampleSchema)
{
}
...
std::string ReservoirProbeLogicalOperator::explain(ExplainVerbosity verbosity) const
{
    ...
    {
        return fmt::format("RESERVOIR_PROBE(opId: {}, statHash: {}, sampleSchema: {})", id, statHash, sampleSchema);
    }
...
void ReservoirProbeLogicalOperator::serialize(SerializableOperator& serializableOperator) const
{
    ...
    (*serializableOperator.mutable_config())[ConfigParameters::STAT_HASH] = descriptorConfigTypeToProto(statHash);
...
{
    auto statHash = std::get_if<uint64_t>(&arguments.config[ReservoirProbeLogicalOperator::ConfigParameters::STAT_HASH]);
    INVARIANT(statHash, "Wrong value in ConfigParameter::STAT_HASH!");
    ...
    auto logicalOperator = ReservoirProbeLogicalOperator(*statHash, sampleSchema);

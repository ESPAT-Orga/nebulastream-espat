...
### Contention mode: shared root-ingress bandwidth caps in Mbit (0 = uncapped baseline). The cap is a
### tbf-policed ingress qdisc on the root container's eth0 (needs --cap-add=NET_ADMIN + tc in the image).
BANDWIDTH_LIMITS_MBIT = [int(x) for x in os.environ.get("BANDWIDTH_LIMITS_MBIT", "0,100,10").split(",")]
INGRESS_BURST = os.environ.get("INGRESS_BURST", "256k")
...
### split (synopsis crosses) land between prometheus (raw) and local (4 scalars). 16 KB << 1.2 MB/window.
HISTOGRAM_MEMORY_BUDGET = int(os.environ.get("HISTOGRAM_MEMORY_BUDGET", 16384))

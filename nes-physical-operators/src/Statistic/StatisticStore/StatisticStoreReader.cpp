...
{
    return index < tProbeStatistics.size() ? tProbeStatistics[index].getNumberOfSeenTuples() : 0;
}

uint64_t getStartTsByIndexProxy(const uint64_t index)
{
    return index < tProbeStatistics.size() ? tProbeStatistics[index].getStartTs().getTime() : 0;
}

uint64_t getEndTsByIndexProxy(const uint64_t index)
{
    return index < tProbeStatistics.size() ? tProbeStatistics[index].getEndTs().getTime() : 0;
}

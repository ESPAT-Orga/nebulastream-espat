...
PlanStage::PlacedLogicalPlan BottomUpOperatorPlacer::place(PlanStage::OptimizedLogicalPlan&& inputPlan) &&
{
    ...
    return PlanStage::PlacedLogicalPlan{
        .plan = LogicalPlan{plan.getQueryId(), {this->placeOperatorRecursive(root)}, plan.getOriginalSql()}};
}
...
BottomUpOperatorPlacer::placeBinaryOperator(const LogicalOperator& op, const LogicalOperator& leftChild, const LogicalOperator& rightChild)
{
    ...
    const auto candidates
        = context.topology->findLCA(*leftChildPlacement, *rightChildPlacement, *sinkPlacement, Topology::Direction::Downstream);
    ...
    context.workerCatalog->consumeCapacity(opPlacement, 1);
...
{
    const auto paths = context.topology->findPaths(src, dest, Topology::Direction::Downstream);
    if (paths.empty())
    {
        throw NoPathFound();
    }
    ...
    {
        if (context.workerCatalog->hasCapacity(nodeId))
        {
            return nodeId;
        }
    }

...
private:
    StatisticId statisticId;
    StatisticType statisticType;
    /// TODO: statistic-renaming stores these as Windowing::TimeMeasure members. Upstream's TimeMeasure
    /// holds `const uint64_t milliSeconds`, so it is not assignable, which would implicitly delete this
    /// class's copy/move assignment and in turn break std::ranges::remove_if in the three statistic
    /// stores. We keep the raw milliseconds privately and reconstruct TimeMeasure in the getters, so the
    /// public API is unchanged. Restore the TimeMeasure members if upstream ever drops the const member.

...
/// Central coordinator for statistic requests. Owns the StatisticRegistry and hands out unique StatisticIds.
///
/// Also runs a gRPC server (StatisticCoordinatorService) that receives results from gRPC sinks. Because the
/// service implementation holds a reference to the coordinator, an instance must not move after
...
public:
    ...
    /// Non-movable and non-copyable: StatisticCoordinatorServiceImpl captures `this`.
    ...
    ///
    /// A probe over a time range emits one row per stored window, and nothing tells the coordinator up front how
...
private:
    ...
    std::unordered_map<Statistic::StatisticId, PendingProbe> pendingProbes;
};

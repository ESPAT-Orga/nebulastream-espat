...
void PrometheusSink::start(PipelineExecutionContext&)
{
    ...
    for (const auto& field : schema.getFields())
    {
        ...
        std::string name = field.name.substr(field.name.find("$") + 1, field.name.size());
        auto gauge = &(prometheus::BuildGauge().Name(name).Help("Derived from nes prometheus sink schema").Register(*registry));

...
EquiWidthHistogramProbeLogicalOperator::EquiWidthHistogramProbeLogicalOperator(const uint64_t statisticHash, DataType counterType, DataType startEndType)
...
EquiWidthHistogramProbeLogicalOperator EquiWidthHistogramProbeLogicalOperator::withInferredSchema(const std::vector<Schema>& inputSchemas) const
{
    auto copy = *this;
...
{
    auto statisticHashValue = std::get_if<uint64_t>(&arguments.config[EquiWidthHistogramProbeLogicalOperator::ConfigParameters::STATISTIC_HASH]);
    INVARIANT(statisticHashValue, "Wrong value in ConfigParameter::STATISTIC_HASH");
    auto counterTypeValue = std::get_if<std::string>(&arguments.config[EquiWidthHistogramProbeLogicalOperator::ConfigParameters::COUNTER_TYPE]);
    INVARIANT(counterTypeValue, "Wrong value in ConfigParameter::COUNTER_TYPE");
    auto startEndType = std::get_if<std::string>(&arguments.config[EquiWidthHistogramProbeLogicalOperator::ConfigParameters::START_END_TYPE]);
    INVARIANT(startEndType, "Wrong value in ConfigParameter::START_END_TYPE");
    auto start = std::get_if<std::string>(&arguments.config[EquiWidthHistogramProbeLogicalOperator::ConfigParameters::BIN_START_FIELD_NAME]);
    ...
    auto counter = std::get_if<std::string>(&arguments.config[EquiWidthHistogramProbeLogicalOperator::ConfigParameters::BIN_COUNTER_FIELD_NAME]);

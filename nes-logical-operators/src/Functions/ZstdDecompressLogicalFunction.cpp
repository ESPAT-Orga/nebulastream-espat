...
LogicalFunction ZstdDecompressLogicalFunction::withInferredDataType(const Schema<Field, Unordered>& schema) const
{
    ...
    INVARIANT(newChildren.size() == 1, "ZstdDecompressLogicalFunction expects exactly one child but has {}", newChildren.size());
...
Reflected Reflector<ZstdDecompressLogicalFunction>::operator()(const ZstdDecompressLogicalFunction& function, const ReflectionContext& context) const
{
    return context.reflect(detail::ReflectedZstdDecompressLogicalFunction{.child = function.child});
}

ZstdDecompressLogicalFunction Unreflector<ZstdDecompressLogicalFunction>::operator()(const Reflected& reflected, const ReflectionContext& context) const
{
    auto [child] = context.unreflect<detail::ReflectedZstdDecompressLogicalFunction>(reflected);
    if (!child.has_value())
    {
        throw CannotDeserialize("ZstdDecompressLogicalFunction is missing its child");
    }
    return ZstdDecompressLogicalFunction{child.value()};
}

LogicalFunctionRegistryReturnType ZstdDecompressLogicalFunction::createZSTD_DECOMPRESS(LogicalFunctionRegistryArguments arguments)
{
    if (arguments.children.empty())
    {
        throw CannotDeserialize("ZSTD_DECOMPRESS requires one argument");
    }
    return ZstdDecompressLogicalFunction(arguments.children.back());
}

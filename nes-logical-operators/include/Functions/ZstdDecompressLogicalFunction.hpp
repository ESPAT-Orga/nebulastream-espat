...
/// Logical function that decompresses a zstd-compressed VARSIZED blob, producing a VARSIZED value.
class ZstdDecompressLogicalFunction final
{
public:
    static constexpr std::string_view NAME = "ZSTD_DECOMPRESS";

    /// NOLINTNEXTLINE(modernize-pass-by-value)
    explicit ZstdDecompressLogicalFunction(const LogicalFunction& child);
    ...
    static LogicalFunctionRegistryReturnType createZSTD_DECOMPRESS(LogicalFunctionRegistryArguments arguments);

private:
    DataType dataType;
    LogicalFunction child;

    friend Reflector<ZstdDecompressLogicalFunction>;
};

template <>
struct Reflector<ZstdDecompressLogicalFunction>
{
    Reflected operator()(const ZstdDecompressLogicalFunction& function, const ReflectionContext& context) const;
};

template <>
struct Unreflector<ZstdDecompressLogicalFunction>
{
    ZstdDecompressLogicalFunction operator()(const Reflected& reflected, const ReflectionContext& context) const;
};

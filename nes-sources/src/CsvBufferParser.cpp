...
CsvRowLayout computeCsvRowLayout(const Schema& schema, const size_t bufferSizeInBytes)
{
    ...
    return CsvRowLayout{
        .fieldByteOffsets = std::move(fieldByteOffsets), .tupleSize = tupleSize, .tuplesPerBuffer = bufferSizeInBytes / tupleSize};
}
    ...
    std::vector<TupleBuffer>& outBuffers)
{
    ...
    const auto& fields = schema.getFields();
    const size_t numberOfFields = fields.size();
    const size_t tupleSize = layout.tupleSize;
    const size_t tuplesPerBuffer = layout.tuplesPerBuffer;
    const auto& fieldByteOffsets = layout.fieldByteOffsets;

...
/// Reproduction tests for "Issue 2: LowerToPhysicalOperators drops additional sink roots and duplicates
/// shared subplans" (see MULTI_ROOT_PLAN_ISSUES.md). The tests optimize a single-sink tree with the real
/// RuleBasedOptimizer, graft a second sink root onto the optimized plan (sharing an operator instance —
/// the deployment model where additional statistic sinks are attached to an already-optimized plan),
/// compile through the QueryCompiler, and assert the EXPECTED CompiledQueryPlan shape; they currently
/// FAIL on main:
/// - apply() passes only newRootOperators[0] to the PhysicalPlanBuilder, silently dropping further sinks,
/// - lowerOperatorRecursively has no memoization, so an operator shared by multiple parents is lowered
///   once per path, duplicating the whole upstream subplan including the source.
/// The graft happens post-optimizer because the per-rule optimizer does not preserve multi-root sharing
/// either (issue 1: shared operators are rewritten once per root with fresh ids).

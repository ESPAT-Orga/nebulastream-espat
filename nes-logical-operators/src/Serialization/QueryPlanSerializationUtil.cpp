...
SerializableQueryPlan QueryPlanSerializationUtil::serializeQueryPlan(const LogicalPlan& queryPlan)
{
    ...
    /// Serialize every operator reachable from any root, deduplicated by OperatorId so shared
    /// subtrees (workload-domain shared source) only emit once.
...
LogicalPlan QueryPlanSerializationUtil::deserializeQueryPlan(const SerializableQueryPlan& serializedQueryPlan)
{
    ...
    /// Multi-root plans (workload-domain) carry multiple sinks; validate each.

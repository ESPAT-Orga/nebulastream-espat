...
"""
Pure source→sink passthrough benchmark.

Deploys a query that contains no filter or transformation operators: data flows directly from
the generator source to the file sink. Used to measure the raw source/sink throughput ceiling
so we can tell whether the ~330k tup/s plateau observed in the filter benchmarks is a property
of the query pipeline or of the source itself.

Same worker config (1024 buffers × 64KB) and source generator settings as the filter scripts.

Usage (run from repository root):
    python -m scripts.benchmarking.adaptive-optimization.run_passthrough_benchmark
    python -m scripts.benchmarking.adaptive-optimization.run_passthrough_benchmark --duration 60
"""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Passthrough baseline: source → sink, no operators in between."
    )

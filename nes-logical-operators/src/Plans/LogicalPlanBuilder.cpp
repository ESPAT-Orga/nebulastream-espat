...
LogicalPlan LogicalPlanBuilder::addStatisticBuild(LogicalPlan queryPlan, const std::shared_ptr<Windowing::WindowType>& windowType,
    ...
    std::shared_ptr<LogicalStatisticFields> logicalStatisticFields)
{
    ...
    if (logicalStatisticFields)
    {
        return promoteOperatorToRoot(
            queryPlan,
            StatisticBuildLogicalOperator(std::move(onKeys), std::move(windowAggs), windowType, std::move(logicalStatisticFields)));
    }
    else
    {

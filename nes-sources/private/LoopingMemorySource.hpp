...
/// A replaying variant of MemorySource for the adaptive-optimization benchmarks. Like MemorySource
/// it pre-formats one or more CSV files into native row-layout TupleBuffers during setup(), but it
/// then replays them: it can alternate between two files, loop indefinitely, and rewrite a monotonic
/// timestamp field so the event-time watermark keeps advancing across loop cycles. Because buffers
/// are replayed they are shared (not moved) on emit, and the timestamp rewrite copies the buffer.
/// Plain single-pass workloads should use MemorySource (type "Memory"), which keeps the zero-copy
/// emit and does not retain the whole dataset for replay.
class LoopingMemorySource final : public Source
{
public:
    static constexpr std::string_view NAME = "LoopingMemory";

    explicit LoopingMemorySource(const SourceDescriptor& sourceDescriptor, size_t bufferSizeInBytes);
    ~LoopingMemorySource() override = default;

    LoopingMemorySource(const LoopingMemorySource&) = delete;
    LoopingMemorySource& operator=(const LoopingMemorySource&) = delete;
    LoopingMemorySource(LoopingMemorySource&&) = delete;
    LoopingMemorySource& operator=(LoopingMemorySource&&) = delete;
...
struct ConfigParametersLoopingMemory
{
    ...
    static inline const DescriptorConfig::ConfigParameter<bool> LOOP{
        "loop", false, [](const std::unordered_map<std::string, std::string>& config) { return DescriptorConfig::tryGet(LOOP, config); }};

...
std::shared_ptr<WindowAggregationLogicalFunction> EquiWidthHistogramLogicalFunction::create(
    const FieldAccessLogicalFunction& onField,
    const FieldAccessLogicalFunction& asField,
    uint64_t numBuckets,
    uint64_t minValue,
    uint64_t maxValue)
{
    return std::make_shared<EquiWidthHistogramLogicalFunction>(onField, asField, numBuckets, minValue, maxValue);
}

std::shared_ptr<WindowAggregationLogicalFunction> EquiWidthHistogramLogicalFunction::create(
    const FieldAccessLogicalFunction& onField, uint64_t numBuckets, uint64_t minValue, uint64_t maxValue)
{
    return std::make_shared<EquiWidthHistogramLogicalFunction>(onField, numBuckets, minValue, maxValue);
}
...
{
    /// TODO Adapt to equi width histogram
